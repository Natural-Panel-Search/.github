...
현재 시스템의 한계를 명확히 인식하고, 지속적인 고도화를 위한 개선 계획을 수립하였습니다.

### 1. 사전 업데이트 자동화 (Dictionary Automation)
- **Current Limitation**: 행동 패턴 사전과 인물 정보 사전이 정적으로 관리되어 최신 트렌드 반영이 늦어질 수 있습니다.
- **Future Plan**: LLM을 활용하여 실시간 유입 데이터를 분석하고, **사전을 자동으로 업데이트 및 확장**하는 파이프라인을 구축할 예정입니다.

### 2. 실시간 정확도 측정 시스템 (Real-time Evaluation)
- **Current Limitation**: 현재 정확도 측정은 사전에 정의된 3가지 기준 쿼리를 통해 수동 검증 방식으로 진행하였습니다.
- **Future Plan**: 사용자 피드백 루프와 로그 분석을 통해 **검색 정확도 점수(Scoring)를 실시간으로 대시보드에 시각화**하는 모니터링 시스템을 도입할 예정입니다.
